...
const MapView: React.FC<MapViewProps> = ({ alerts, userLocation, onRespond }) => {
  ...
  if (!userLocation && alerts.length === 0) {
     return <div className="h-full flex items-center justify-center bg-gray-100 text-gray-500">Waiting for location...</div>;
  }
  ...
  return (
    <div className="h-[600px] w-full rounded-2xl overflow-hidden border border-gray-200 shadow-md relative z-0">
      <MapContainer 
      ...
      <div className="absolute bottom-4 left-4 bg-white p-3 rounded-lg shadow-xl z-[400] text-xs space-y-2 border border-gray-100">
        <div className="font-bold text-gray-900 mb-1">Legend</div>
