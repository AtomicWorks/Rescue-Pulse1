...
interface AlertCardProps {
  ...
  onComments?: (alertId: string) => void;
      ...
      <div className="flex flex-wrap gap-2.5 mt-2 pl-2">
        ...
        {!isCurrentUser && (
          <>
            ...
            {onComments && (
              <button
                onClick={() => onComments(alert.id)}
                className={`flex-1 min-w-[80px] py-2.5 px-4 rounded-xl text-xs font-bold transition-all border flex items-center justify-center gap-1.5 ${d ? 'bg-slate-800 text-slate-300 hover:bg-slate-700 border-white/5' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 border-slate-200'
                  }`}
              >
